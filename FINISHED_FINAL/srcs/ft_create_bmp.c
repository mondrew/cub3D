...
/*   Updated: 2020/07/14 12:28:54 by mondrew          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		ft_put_bmp(int fd, t_cub *scene)
{
	int	i;
	int	j;
...
static int		ft_write_bmp_header(int fd, t_bfh *file_header, \
		t_bih *info_header)
{
...
static int		ft_close_file(int fd)
{
	if (close(fd) == -1)
	{
		ft_putstr_fd("Error\nclose() failed\n", 1);
...
static void		ft_bmp_structs(t_bfh *file_header, t_bih *info_header, \
		t_cub *scene)
{
