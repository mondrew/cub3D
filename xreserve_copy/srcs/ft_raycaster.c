...
/*   Updated: 2020/06/27 19:18:21 by mondrew          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		ft_draw_the_line(int x, double line_len, t_cub *scene)
{
	...
	ny = 0;
	line_len *= cos(scene->ray_angle - scene->plr_angle);
	real_height = (64 / line_len) * scene->dist_to_pp;
	if (real_height > scene->res_y)
		screen_h = scene->res_y;
	else
		screen_h = real_height; // ROUNDED UP + 1??
	y = scene->res_y / 2 - screen_h / 2;
	if (scene->line_len_h < scene->line_len_v)
	{
		if (scene->dir_y >= 0)
			color = 0x000000ff; // S - blue
		if (scene->dir_y < 0)
			color = 0x00ff0000; // N - red
	}
	if (scene->line_len_h > scene->line_len_v)
	{
		if (scene->dir_x >= 0)
			color = 0x00ffff00; // E - yellow
		if (scene->dir_x < 0)
			color = 0x00008000; // W - green
	}
	while (ny < y)
	{
